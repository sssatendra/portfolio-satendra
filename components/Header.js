...
function Header({ data }) {
    ...
    return (
        <header className="sticky top-0 z-50 items-center h-16" style={{ backgroundColor: "#222222" }}>
            <div className="flex justify-between items-center w-full my-10">
                <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 2 }} className="flex space-x-5 w-72 text-white justify-center items-center" >
                    <h1 className="p-3 text-xl cursor-pointer w-14 text-center rounded-full border-2 font-extrabold bg-blue-500 "> {data.main.initial} </h1>
                ...
                <div className="lg:hidden">
                    <MenuIcon onClick={handleClick} className="text-white sticky  lg:hidden h-8 cursor-pointer md:inline-flex mr-8" />
                </div>
            </div>
            <div className="transition transform duration-700 relative">
                {
                    show && (
                        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="absolute top-0 bg-black right-0 -mt-10 rounded-md ">
